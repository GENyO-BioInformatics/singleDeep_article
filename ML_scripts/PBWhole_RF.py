...
param_grid = {
    ...
    "max_features": ["sqrt"] + list(np.arange(10, 101, 10)),
    "bootstrap": [True],
}
...
for foldOut, (train_index, test_index) in enumerate(outer_cv.split(expression, metadata["LabelInt"].tolist())):
    ...
    random_search = RandomizedSearchCV(
        rf_model, param_grid, n_iter=100, scoring=make_scorer(matthews_corrcoef), cv=inner_cv, n_jobs=cores,
        random_state=123
    )
    ...
    best_model = RandomForestClassifier(**best_params, class_weight='balanced', random_state=123)
...
modelCluster = RandomForestClassifier(**best_params, class_weight='balanced', random_state=123)
